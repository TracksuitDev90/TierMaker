/* =========================================================
   FSTM Tier Maker — Full JavaScript
   (2025-08)  —  fixes: capture, label sizing, row drag, help,
   color picker UX, autosave, undo, a11y, polyfills.
========================================================= */
...
/* ---------- Palettes ---------- */
/* Black labels on pre-rendered names; slightly *less* pale (20% less lift). */
var BASE_PALETTE = [
...
function ensureForBlack(hex){
  // Lift until readable against black; gentler steps (less pale overall)
  var out=hex, step=0;
  while (step<3) {
    var L=relativeLuminance(hexToRgb(out));
    if ((L+0.05)/0.05 >= 4.5) break;
    out = lighten(out, 0.03); // gentler than earlier 0.0375
    step++;
  }
  return out;
}
...
/* ---------- Size-to-fit helpers ---------- */
/* Circle token labels (UI): target big single line that fits the circle */
function fitTokenLabel(labelEl, tokenEl){
  ...
  var min = Math.max(12, Math.floor(D * 0.2)); // do not go too tiny
  var max = Math.floor(D * 0.42);              // keeps inside the circle
  var best = min;
...
/* Tier chip text shrink-to-fit (contenteditable) */
function fitChipText(chip){
  ...
  var W = chip.clientWidth - pad*2, H = chip.clientHeight - 10;
  var min = 11, max = 28, best = min;
  function fits(px){
...
function buildTokenBase(){
  ...
  on(el,'keydown', function(e){
    // a11y reordering with Alt+Arrows
...
function buildRowDom(){
  ...
  // Make the label area the row drag handle (pointer-based)
...
function enableRowReorder(handle, row){
  var dragging=false, placeholder=null, startY=0;
...
function openRadial(token){
  ...
  // compact arc, uniform on both sides
  ...
  positions.forEach(function(pos, j){
    ...
    on(btn,'click', function(){ selectRadialTarget(rows[j]); });
    radialOpts.appendChild(btn);
    _radialGeo.push({btn:btn,row:rows[j]});
  });
...
/* ---------- History (Undo) ---------- */
var historyStack = []; // {itemId, fromId, toId, beforeId, type:'move'|'rows'}
...
on($('#undoBtn'),'click', function(){
  ...
  if (last.type==='move'){
    ...
    // place rows back to previous order
...
on($('#saveBtn'),'click', function(){
  ...
  // export-only CSS: center labels perfectly; DO NOT change sizes
  var style = document.createElement('style');
  style.textContent = `
    .row-del{ display:none !important; } /* don’t show X in PNG */
    .token .label{
  ...
  // remove title if it’s blank/placeholder
...
/* ---------- Autosave (restore unless navigation is reload) ---------- */
function snapshot(){
...
/* ---------- Help copy (clean lines) ---------- */
function setHelpCopy(){
  var help=$('#helpText') || $('.help'); if(!help) return;
  help.innerHTML =
    '<strong>Help</strong>' +
    '<div style="margin-top:6px"><strong>Desktop / Tablet:</strong> drag circles into rows. Reorder by dragging items or with <kbd>Alt</kbd> + arrow keys.</div>' +
    '<div><strong>Phone:</strong> tap a circle in Image Storage to choose a row. Once placed, drag to reorder or drag back to Image Storage.</div>' +
    '<div><strong>Rows:</strong> drag the tier label to reorder rows. Click the tier name to edit it. Tap the small X on a tier label to delete that row (its items return to Image Storage).</div>';
}

/* ---------- Color picker UX ---------- */
(function(){
  var input = $('#nameColor'); if(!input) return;
  // show current value in the input ring (the input already previews color)
  on(input,'input', function(){ input.setAttribute('aria-label','Selected color '+input.value); });
})();

/* ---------- History + autosave on row structural changes ---------- */
on(document,'pointerup', function(e){
  // If a row was reordered, take a snapshot for undo
  // We can’t easily detect “which one moved” here across all paths, so just record order.
  if (e.target.closest && e.target.closest('.tier-label')) recordRowOrder();
});
...
document.addEventListener('DOMContentLoaded', function start(){
  ...
  on($('#addNameBtn'),'click', function(){
    ...
    // keep chosen color (don’t silently change)
    $('#nameInput').value='';
    scheduleSave();
  });
  ...
  // Tray click-to-place enabled
  enableClickToPlace(tray);

  // Theme button ARIA text toggles in CSS/HTML; nothing else needed here

  // Help
  setHelpCopy();

  // Enable Undo button state
  $('#undoBtn').disabled = historyStack.length===0;
