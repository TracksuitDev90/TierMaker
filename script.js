/* =========================================================
   Tier Maker — JS (7-upgrade build, trimmed for your 4 asks)
   - PNG export = exact on-screen sizes; labels centered in PNG
   - No row reordering (only circles reorder)
   - Tier labels shrink-to-fit while editing
   - Help tips spaced & headed
   - Keeps: polyfills, autosave, undo (for token moves & row delete),
            color picker preview, a11y, mobile radial
========================================================= */
...
/* ---------- Tier chip shrink-to-fit ---------- */
function fitChipText(chip){
  var pad = 14, W = chip.clientWidth - pad*2, H = chip.clientHeight - 8;
  var min = 11, max = 24, best = min; // stay big if possible
  function fits(px){ chip.style.fontSize=px+'px'; return chip.scrollWidth<=W && chip.scrollHeight<=H; }
...
function buildTokenBase(){
  ...
  // keyboard: Alt+←/→ reorder within row, Alt+↑/↓ move rows
...
/* ---------- History (Undo) ---------- */
var historyStack = []; // {type:'move',...} | {type:'row-delete', rowHTML}
...
function openRadial(token){
  ...
  for (var i=0;i<N;i++){
    (function(j){
      ...
      btn.style.left = x+'px'; btn.style.top  = y+'px'; btn.style.transitionDelay=(j*14)+'ms';
      var dot=document.createElement('span'); dot.className='dot'; dot.textContent=labels[j]; btn.appendChild(dot);
      on(btn,'click', function(){ moveToken(token, rows[j].querySelector('.tier-drop'), null); closeRadial(); });
...
on($('#undoBtn'),'click', function(){
  ...
  if (last.type==='move'){
    ...
    if(container){
      ...
      var chip=$('.label-chip',row), del=$('.row-del',row), drop=$('.tier-drop',row);
      on(chip,'keydown', function(e){ if(e.key==='Enter'){ e.preventDefault(); chip.blur(); } });
      on(chip,'input', function(){ fitChipText(chip); queueAutosave(); });
      on(del,'click', function(){
...
/* ---------- Export: exact sizes; center labels ONLY in PNG ---------- */
(function(){
  // little overlay
  ...
  on($('#saveBtn'),'click', function(){
    ...
    // Only for export: hide row X; absolutely center labels so they can’t be pushed down by line-height quirks.
    var s = document.createElement('style');
    s.textContent = `
      .row-del{ display:none !important; }
      .token{ position:relative !important; }
      .token .label{
    ...
    // If title is blank, drop its wrapper in the clone
...
/* ---------- Help tips (spaced & headed; no row-reorder mention) ---------- */
function setHelpCopy(){
  ...
  help.innerHTML =
    '<strong>Help</strong>' +
    '<div style="margin-top:6px"><strong>Desktop / Tablet:</strong> drag circles into rows. Reorder by dragging items or use <kbd>Alt</kbd> + arrow keys.</div>' +
    ...
    '<div><strong>Tiers:</strong> click a tier name to edit it; the text will shrink to fit as you type. Tap the small X on a tier label to delete that row (its items return to Image Storage).</div>';
}
...
document.addEventListener('DOMContentLoaded', function start(){
  ...
  // Restore or build defaults
  var saved=null; try{ saved = JSON.parse(localStorage.getItem(AUTOSAVE_KEY)||'null'); }catch(_){}
  if (saved && restoreState(saved)){ announce('Restored your last session.'); }
  else {
  ...
  on($('#addNameBtn'),'click', function(){
    ...
    nameInput.value='';
    colorInput.value = nextPreset(); // rotate palette
